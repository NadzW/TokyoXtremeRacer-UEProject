...
namespace Alpakit.Automation
{
	...
	public class PackagePlugin : BuildCommand
	{
		...
		private const string ModDeployRoot = "Mods";

		private static string GetGameLaunchURL(BuildCommand cmd)
		{
			return "steam://rungameid/20139094";
		}
		...
		private static void CopyPluginToTheGameDir(string gameDir, FileReference projectFile, FileReference pluginFile,
			string stagingDir)
		{
			// <GameDir>/<ProjectName>/<ModDeployRoot>
			var modsDir = Path.Combine(gameDir, projectFile.GetFileNameWithoutAnyExtensions(),  ModDeployRoot);
			
			var projectModsFolder = new DirectoryReference(Path.Combine(projectFile.Directory.ToString(), ModDeployRoot));
			var pluginRelativePath = pluginFile.Directory.MakeRelativeTo(projectModsFolder);
		...
		private static ProjectParams GetParams(BuildCommand cmd)
		{
			...
			var projectParameters = new ProjectParams(
				...
				Stage: true,
				DLCName: pluginName,
				BasedOnReleaseVersion: "SOMERELEASE1"
            );
		...
		private static void RemapCookedPluginsContentPaths(ProjectParams projectParams, IEnumerable<DeploymentContext> deploymentContexts) {
			foreach (var deploymentContext in deploymentContexts) {
				//We need to make sure content paths will be relative to ../../../ProjectRoot/<ModDeployRoot>/DLCFilename/Content,
				...
				var destinationModsDir = Path.Combine(projectName, ModDeployRoot);
		...
		private static string GetPluginPathRelativeToStageRoot(ProjectParams projectParams, DeploymentContext SC)
		{
			...
			return Path.Combine(projectName, ModDeployRoot, dlcName);
		}
		...
		public override void ExecuteBuild()
        {
			...
			{
				var bootstrapExePath = Path.Combine(gameDir, "TokyoXtremeRacer.exe");
			...
			try
			{
				...
				//PackagePluginProject(deploymentContexts, "");
